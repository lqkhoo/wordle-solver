...
class WordleSolutionTree(object):
    ...
    def _build_tree_inner(self, parent_id: int, key: int, live_cidxs: np.ndarray, dft_mode=False) -> None:
       ...
       # Make guess
        ...
        for key in range(n_keys):
            ...
            if size == 0 or key == n_keys-1:
                ...
                continue
            if dft_mode:
                self.stack.append((node_id, key, new_live_cidxs))
            else:
        ...
        if self.verbose:
            print("Guess {}: {}. n_unexplored_nodes: {}. Elapsed: {}".format(
                node_id,
                guess_raw,
                self.queue.qsize() if not dft_mode else len(self.stack),
                self.elapsed
            ))


    def build_tree(self, mode: str='dft') -> None:
        ...
        if mode == 'bft':
            ...
            while not self.queue.empty():
                ...
                self._build_tree_inner(parent_id, key, live_cidxs, dft_mode=False)

        elif mode == 'dft':
            self.stack.append((None, None, live_cidxs))
            while not len(self.stack) == 0:
                parent_id, key, live_cidxs = self.stack.pop()
                self._build_tree_inner(parent_id, key, live_cidxs, dft_mode=True)
    ...
    def build_tree_concurrent(self, mode: str='dft', n_streams: int=16):

        # Create pool
        pool = []
        for _ in range(n_streams):
            pool.append(cp.cuda.Stream())
